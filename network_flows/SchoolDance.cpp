...
int main(void) {
  ...
  while (k--) {
    int u, v;
    cin >> u >> v;
    graph.AddEdge(u, n + v, 1, true);
  }
  for (int i = 1; i <= n; i++) graph.AddEdge(0, i, 1, true);
  for (int i = 1; i <= m; i++) graph.AddEdge(n + i, nodes - 1, 1, true);
  int max_flow = graph.MaxFlow(0, nodes - 1, nodes);
  cout << max_flow << '\n';
  for (int i = 1; i <= n; i++) {
    for (int j = 1; j <= m; j++) {
      if (graph.capacity[i][n + j] > 0 && graph.flow[i][n + j] == 1) {
        cout << i << " " << j << '\n';
      }
    }
  }
